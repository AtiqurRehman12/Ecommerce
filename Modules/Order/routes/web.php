...
Route::group(['namespace' => '\Modules\Order\Http\Controllers\Backend', 'as' => 'backend.', 'middleware' => ['web', 'auth', 'can:view_backend'], 'prefix' => 'admin'], function () {
    ...
    */
    ...
    Route::get('/orders/detail/{id}', ['as' => "$module_name.detail", 'uses' => "$controller_name@detail"]);
    Route::get('/orders/detail/confirm/{id}', ['as' => "$module_name.confirm", 'uses' => "$controller_name@confirm"]);
    Route::get('/orders/detail/reject/{id}', ['as' => "$module_name.reject", 'uses' => "$controller_name@reject"]);
